...
int main()
{
    ...
    for(int i=0;i<N;i++)
    {
        ...
        if(position == -1)
        {
            ...
            valueList.push_back(make_pair(maxVal4EachCnt.size()-1, val[i]));
        }
        else
        {
            maxVal4EachCnt[position] = val[i];
            valueList.push_back(make_pair(position, val[i]));
        }
    }
    cout << maxVal4EachCnt.size() << endl;
    int startIdx = maxVal4EachCnt.size()-1;
    vector<int> result;
    for(int i=valueList.size()-1;i>=0;i--)
    {
        if(startIdx == valueList[i].first)
        {
            result.push_back(valueList[i].second);
            startIdx --;
        }
    }
